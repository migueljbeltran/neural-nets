...
Adapted from the instructor-provided template (Jiawei Zhang) to the repo's
src.base_class layout. Loads a citation network (cora / citeseer / pubmed),
builds the symmetrically normalized adjacency matrix with self-loops, and
returns torch tensors plus train/val/test index splits.
'''
...
class Dataset_Loader(dataset):
    ...
    def load(self):
        ...
        # train / val / test index splits (standard public planetoid splits)
        ...
        print('  nodes: {}, features: {}, classes: {}, edges: {}'.format(
            features.shape[0], num_features, num_classes, edges.shape[0]))
        print('  train/val/test sizes: {}/{}/{}'.format(
